...
function buildLines(): Line[] {
  ...
  // RIGHT array order: [F4(col6), E8(col7), S16(col8), R32(col9), R64(col10)]
  // Connector between RIGHT[i+1] (outer) → RIGHT[i] (inner): arm goes left from outer col.
            ...
            colMatchups(cfg, matchups).map((m, j) => {
              ...
              return (
                <div
                  ...
                  className="border border-gray-200 rounded bg-white overflow-hidden shadow-none"
                >
                  <div
                    className={`px-1.5 truncate flex items-center border-b border-gray-100 ${
                      aWin    ? 'font-black text-[#C9A000] bg-yellow-50'
                      : decided ? 'text-gray-300 line-through'
